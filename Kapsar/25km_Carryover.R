...
# Tasks
# Move over 25km data from original Kapsar dataset into the new split dataset
...
source_packageId <- "resource_map_doi:25.18739/A2P55DJ23"
...
## -- read in target data -- ##
target_packageId <- "resource_map_urn:uuid:8031dcfe-79ff-4cdc-8044-5e888e546fd5"
...
#     paste("Monthly vessel intwentysity within 1km pixel of coastlines of the study area during", 
...
twenty <- which_in_eml(source_doc$dataset$spatialRaster, "entityName", 
                    function(x) {
                      grepl("25km", x)
                    })
...
twenty <- source_doc$dataset$spatialRaster[twenty]
...
# need to reverse twenty
twenty <- rev(twenty)


# Step 6: Copy over the attributes
for (i in 1:length(target_doc$dataset$otherEntity)){ 
  target_doc$dataset$otherEntity[[i]]$attributeList <- twenty[[i]]$attributeList
}
...
twenty_folder <- "Kapsar/25km"

# create list of the file names
twenty_filenames <- list.files(twenty_folder, full.names = TRUE)
...
spatialRaster <- vector("list", length(twenty_filenames))

# create spatial raster entities
for(i in 1:length(twenty_filenames)){
  spatialRaster[[i]] <- dvk_get_raster_metadata(twenty_filenames[i],
...
## -- update package -- ##
eml_path <- "~/Scratch/North_Pacific_and_Arctic_Marine_Vessel_Traffic_25km.xml"
...
pids <- arcticdatautils::get_package(d1c@mn,
                   "resource_map_urn:uuid:405b0eef-057a-403b-90f0-2206561205fb")


for(i in 1:length(pids$data)){
  sysmeta <- getSystemMetadata(d1c@mn, pids$data[[i]])
  sysmeta@formatId <- "image/geotiff"
  updateSystemMetadata(d1c@mn, pids$data[[i]], sysmeta)
}
